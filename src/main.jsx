...
function DashboardCargaDetail({options,tasks,liveTasks=[],users=[],selected,setSelected,statusById}){
  ...
  const rows=options.map(item=>{
    ...
    return {...item,tasks:itemTasks,edit,alter,total,online,activeTask};
  ...
  return <div className="dashboard-detail panel dashboard-entity-list dashboard-carga-list">
    {rows.map(row=>{
      ...
      return <div className="dashboard-entity-group" key={row.id}>
        <button type="button" className={`dashboard-entity-summary carga-entity-summary${expanded?' active':''}`} aria-expanded={expanded} onClick={()=>setSelected(expanded?'':row.id)}>
          <span className="dashboard-entity-name"><span className="dashboard-chevron">{expanded?'⌄':'›'}</span>{row.avatar&&<AvatarMini value={row.avatar} label={row.name}/>}<strong>{row.name}</strong></span>
          <span className="carga-status">{row.online&&<><span className="carga-online-tag">Online</span>{row.activeTask&&<span className="carga-active-task">{row.activeTask.title}</span>}</>}</span>
          <span className="dashboard-stat">{row.tasks.length} tarefa(s)</span>
          <span className="dashboard-stat">{fmtSec(row.total)}</span>
        </button>
