...
function App(){
  ...
  useEffect(()=>{
    ...
    const scoped=(notifications||[]).filter(n=>n?.userId===auth.id&&!isPushOnlyNotification(n));
...
function Sidebar({auth,effectiveUser,viewAs,setViewAs,users,companies=[],notifications=[],system,realAdmin,nav,screen,setScreen,setAuth,notificationAlertsEnabled,notificationPermission,enableNotificationAlerts,disableNotificationAlerts,sidebarCollapsed,setSidebarCollapsed}){
  ...
  const pendingNotificationsCount = (notifications||[]).filter(n=>n?.userId===effectiveUser?.id && !n?.done && !isPushOnlyNotification(n)).length;
...
function TaskPage({task,tasks=[],setTasks,companies,users,statuses,types,statusById,updateTask,addLog,back,open,effectiveUser,isAdmin}){ 
  ...
  function retractVote(){
    ...
    updateTask(task.id,{approvalVotes:votes},`${effectiveUser.name} retirou sua decisão (${votes.length} de ${approversForCompany.length} responderam).`);
  ...
  function retractCopyVote(){
    ...
    updateTask(task.id,{copyApprovalVotes:votes},`${effectiveUser.name} retirou sua decisão sobre a copy (${votes.length} de ${approversForCompany.length} responderam).`);
  ...
  return <div className="modal-bg"><div className="modal"><ModalDismiss onClose={cancel}/><h2>Configurações de {f.name||'usuário'}</h2>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      ...
      <AccessConfigCard title="Notificações e alertas" open={openSection==='rule:notifications'} onToggleOpen={()=>toggleSection('rule:notifications')} accent customized={notificationRulesDiff}>
        <div style={notificationInheritance!=='custom'?{pointerEvents:'none'}:undefined}>
          <p className="muted" style={{marginTop:0}}>"Sistema" e "Push" são canais independentes. Status continuam limitados por "Status disponíveis".</p>
    ...
    {mode==='order'?<div className="panel access-order-panel"><h2>Ordenação dos painéis</h2><p className="muted">Define a ordem-base da barra lateral para todos os perfis. Financeiro e Configurações aparecem somente para Admin, mas também respeitam esta ordem.</p><div className="access-order-list">{panelOrder.map((id,index)=>{const panel=SIDEBAR_PANEL_CATALOG.find(item=>item.id===id);return <div className="access-order-row" key={id}><b>{index+1}. {panel?.label||id}</b><div><button onClick={()=>moveSidebarPanel(index,-1)} disabled={index===0}>↑ Subir</button><button onClick={()=>moveSidebarPanel(index,1)} disabled={index===panelOrder.length-1}>↓ Descer</button></div></div>})}</div><div className="modal-actions"><button onClick={restorePanelOrder}>Restaurar ordem</button><button className="primary" onClick={savePanelOrder}>Salvar ordenação</button></div></div>:<div className="panel"><h2>Padrão de {roleLabel}</h2>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        ...
        <AccessConfigCard title="Notificações e alertas" open={openSection==='rule:notifications'} onToggleOpen={()=>toggleSection('rule:notifications')} accent>
          <p className="muted" style={{marginTop:0}}>Escolha os canais de forma independente. Um usuário pode receber Push mesmo sem receber a notificação no painel.</p>
          <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) 72px 72px',gap:8,alignItems:'center'}}>
            <small className="muted">Evento</small><small className="muted" style={{textAlign:'center'}}>Sistema</small><small className="muted" style={{textAlign:'center'}}>Push</small>
...
function NotificationDeliverySettings({currentUser,statuses,role='team'}){
  ...
  async function saveDelivery(){
    try{
      ...
      const normalizedForm={...form,rules:(form.rules||[]).map(rule=>({...rule,targets:(rule.targets||[]).filter(target=>target!=='admins')}))};
  ...
  return <div>
    ...
    {message&&<div className="public-portfolio-success">{message}</div>}
    <div style={{display:'flex',flexDirection:'column',gap:8}}>
      {relevantRules.map(rule=>{const status=(statuses||[]).find(item=>item.id===rule.statusKey);const hours=Math.max(1,Math.round(Number(rule.firstAfterMinutes||1440)/60));return <div key={`${role}:${rule.statusKey}`} className="panel" style={{padding:10}}>
...
function NotificationsPage({notifications,setNotifications,open,tasks,companies,users,statuses,user,alertsEnabled,notificationPermission,enableAlerts}){ 
  ...
  const scoped=notifications.filter(n=>(!n.userId||n.userId===user.id)&&!isPushOnlyNotification(n));
    ...
    <div className="notifications-list">{list.length?notificationGroups.map((group,groupIndex)=><div className="panel notification-company-group" key={`${group.companyKey}-${groupIndex}`}>
      ...
      <div className="notification-list-main">
        <div className="notification-copy">
          ...
          <div className="notification-copy-line notification-copy-message" title={`${info.actorName?`${info.actorName}: `:''}${presentation.type==='status'?`${presentation.from} → ${presentation.to}`:(presentation.text||'')}`}>
